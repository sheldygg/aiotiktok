...
class TikTokClient:
    ...
    async def _make_request(self, method: HTTPMethod, endpoint: str, **kwargs) -> Tuple[dict, int]:
    ...
    async def get_video_data(
        ...
        data, response_status = await self._make_request(
            HTTPMethod.GET, "aweme/v1/feed/", params=params
        )
        aweme_raw_data = data["aweme_list"][0]

        return Aweme.from_dict(aweme_raw_data, status=response_status)
